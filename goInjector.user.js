...
(function () {
      ...
      xhr.onreadystatechange = function () {
        if (xhr.readyState === 4) {
          if (xhr.status === 200) {
            ...
            reject(new Error(`get venue type failed, status: ${xhr.status}`));
          }
        }
      };

      xhr.onerror = () => {
        reject(new Error("get venue type failed"));
      };
  ...
  async function getVenueType() {
    let vt = localStorage.getItem("venueType");
    if (!vt) {
      vt = await requestVenueType();
      localStorage.setItem("venueType", vt);
    }
    return JSON.parse(vt);
  }

  function requestVenue(venueTypeId) {
    return new Promise((resolve, reject) => {
      const xhr = new XMLHttpRequest();
      xhr.open(
        "GET",
      ...
      xhr.onreadystatechange = function () {
        if (xhr.readyState === 4) {
          if (xhr.status === 200) {
            ...
            reject(new Error(`get venue failed, status: ${xhr.status}`));
          }
        }
      };

      xhr.onerror = () => {
        reject(new Error("get venue failed"));
      };
      ...
      xhr.onreadystatechange = function () {
        if (xhr.readyState === 4) {
          if (xhr.status === 200) {
            ...
            reject(new Error(`get me failed, status: ${xhr.status}`));
          }
        }
      };

      xhr.onerror = () => {
        reject(new Error("get me failed"));
      };
  ...
  function requestCreditFee(VenueTypeId) {
    return new Promise((resolve, reject) => {
      const xhr = new XMLHttpRequest();
      xhr.open(
        "GET",
        "https://gym.sysu.edu.cn/api/venuetype/" +
        VenueTypeId +
        "/feetemplates",
        true
      );
      ...
      xhr.onreadystatechange = function () {
        if (xhr.readyState === 4) {
          if (xhr.status === 200) {
            const result = JSON.parse(xhr.responseText).find(
              (item) => item.UserRole === "学生"
            ).CreditFee;
            resolve(result);
          } else {
            reject(new Error(`get credit fee failed, status: ${xhr.status}`));
          }
        }
      };

      xhr.onerror = () => {
        reject(new Error("get credit fee failed"));
      };
  ...
  async function generateBookingInfo() {
    ...
    if (venueConfig && venueConfig.VenueTypeId && venueConfig.VenueId) {
      try {
        const VenueTypeInfo = (await getVenueType()).find(
          (item) => item.Identity === venueConfig.VenueTypeId
        );
        const VenueInfo = JSON.parse(
          await requestVenue(venueConfig.VenueTypeId)
        ).find((item) => item.Identity === venueConfig.VenueId);

        let startDateTime, endDateTime;

        if (venueConfig.timeSlot && venueConfig.bookingDate) {
          let timeSlot = JSON.parse(venueConfig.timeSlot);
          startDateTime = new Date(
            venueConfig.bookingDate + "T" + timeSlot.startTime.split("T")[1]
          );
          endDateTime = new Date(
            venueConfig.bookingDate + "T" + timeSlot.endTime.split("T")[1]
          );
        } else {
          let now = new Date();
        ...
        if (venueConfig.isInitiator) {
          let BookingInfo = {
            ...
            ActionedBy:
              ...
              venueConfig.UserId ||
              Me.UserId,
            Charge: await requestCreditFee(venueConfig.VenueTypeId),
            IsCash: false,
          };
  ...
  function initializeUI(venueType) {
    let participants = [];
  ...
  `;
      ...
      <div id="manualDateGroup" style="display:none">
        ...
        <div class="input-group">
          ...
          <div class="date-picker">
            <input type="date" id="config-bookingDate" value="${new Date().toISOString().split("T")[0]
      }">
          </div>
        </div>
      </div>
    ...
    function showDialog() {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          document
            ...
            .dispatchEvent(new Event("change"));
          setTimeout(() => {
            document.getElementById("config-timeSlot").value =
      ...
      document.getElementById("config-ActionedBy").value =
        savedConfig.ActionedBy;
    }
    ...
    document
      ...
      .addEventListener("click", async () => {
        ...
        if (document.getElementById("config-autoDate").value === "1") {
          savedConfig.bookingDate = "";
          savedConfig.timeSlot = "";
        }
    ...
    document
      ...
      .addEventListener("change", function (e) {
        ...
        document.getElementById("config-Name").value =
          document.getElementById("config-Name-disabled").value || "";
      });
    ...
    document
      ...
      `;
          ...
          .catch((error) => {
            ...
            venueSelect.disabled = true;
            venueSelect.innerHTML =
              '<option value="">获取场地失败，请重试</option>';
          });

        if (
          venueType.find((res) => res.Identity === e.target.value)
